...
// ============================================================================
const SPREADSHEET_ID = "1jEdC_Test_NMc2a5F36xE2MJfgxMZiZFVfeDqnCdVizNGIMo"; // Reemplazar con ID real
const DRIVE_FOLDER_ID = '1-8QqhS-Test_wtEFFwyBG8CmnEOp5i8rxSM-2'; // Reemplazar con ID real
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        if (userRow[COLS.nombreUsuario - 1] === username && userRow[COLS.password - 1] === password) {
            ...
            headers.forEach((header, index) => {
                if(header !== 'password') { // No devolver la contraseña
                   user[header] = userRow[index];
                }
            });
            user.sessionToken = sessionToken;
            user.id = userRow[COLS.id - 1]; // Asegurarse de que el ID se incluya
